...
extends JpaRepository<SampleUserEntity, String> { // PK의 타입
    
    //# Query Method :
    // - Repository의 메서드 선언 시에 지정된 패턴에 따라 메서드명을 작성하면 JPA가 SQL을 만들어주는 방법
    // - findBy : 필드명을 기준으로 모든 컬럼을 조회할 때 사용,
    //            findBy 뒤에 필드명을 붙여서 작성, 필드명의 첫글자는 대문자이어야 함
    List<SampleUserEntity> findByName(String name);
    SampleUserEntity findByTelNumber(String telNumber);

    //& - And/Or 
    // : and 연산 혹은 or 연산에 사용됨, 필드와 필드 사이에 사용
    //* */  And/Or의 우선순위 주의
    ...
    // - @Query에 있는 nativeQuery 속성을 반드시 true로 지정
    @Query(value=
    "SELECT * " + 
    "FROM sample_user " +
    "WHERE name = :name " +
